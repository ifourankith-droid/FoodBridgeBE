...
- **Phase 9's "8 endpoints" resolved as: `GET /api/admin/dashboard`, `GET /api/admin/listings`, `GET /api/admin/accounts`, `PATCH /api/admin/accounts/{id}/verify`, `PATCH /api/admin/accounts/{id}/suspend` (5) + `GET /api/disputes`, `PATCH /api/disputes/{id}/resolve` (2) + `GET /api/reports/platform` (1).** CLAUDE.md's phrase "Dashboard stats, listings/accounts management, verify/suspend, disputes, platform reports" doesn't give an exact count per group; this is a stated, explicit assumption per the working agreement ("if a requirement is ambiguous, state your assumption in one line and proceed"), not a guess left undocumented. **Raising a dispute is deliberately not built** — no earlier phase wired a user-facing "report an issue" endpoint (`Disputes.RaisedByUserId` exists in the Phase 1 schema, but nothing populates it), and Phase 9 is explicitly titled "Admin module" — adding a non-admin-authored write endpoint here would be scope creep beyond what any phase actually asked for. Verified live by inserting a dispute row directly via SQL, then exercising list/resolve through the real API.
